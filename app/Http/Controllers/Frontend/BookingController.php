...
class BookingController extends Controller
{
    public function Checkout(){
        ...
        if (Session::has('book_date')) {
           ...
           return view('frontend.checkout.checkout',compact('book_data','room','nights'));
            ...
            $notification = array(
                'message' => 'Your booking session has expired or is incomplete. Please start again.',
                'alert-type' => 'error'
            ); 
    ...
    public function BookingStore(Request $request){
        ...
        if ($request->available_room < $request->number_of_rooms) {
            ...
            $notification = array(
                'message' => 'The number of rooms exceeds availability.',
                'alert-type' => 'error'
            ); 
    ...
    public function CheckoutStore(Request $request){
        ...
        $notification = array(
            'message' => 'Booking added successfully. A confirmation will be sent to your email once it is confirmed.',
            'alert-type' => 'success'
        ); 
    ...
    {
        // Fetch the booking and associated room assignments
        $booking = Booking::find($id);
    
        if ($booking) {
            // Delete associated room assignments
    ...
    public function UpdateBookingStatus(Request $request, $id)
    {
        ...
        /// Start Sending Email if Booking Status is 'Confirmed'
        if ($booking->status == '1') { // Adjust the condition as per your application logic
            $data = [
                ...
                'email' => $booking->email,
                'phone' => $booking->phone,
                'booking_no' => $booking->code,
                'room_type' => $booking->room['type']['name'],
                'total_room' => $booking->number_of_rooms,
                'total_guest' => $booking->persion,
                'subtotal' => $booking->total_night * $booking->number_of_rooms * $booking->actual_price,
                'discount' => $booking->discount,
                'total' => $booking->total_night * $booking->number_of_rooms * $booking->actual_price - $booking->discount,
            ];
     ...
     public function AssignRoomStore($booking_id,$room_number_id){
        ...
        return redirect()->back()->with($notification);   
            ...
            $notification = array(
                'message' => 'Room(s) already assigned',
                'alert-type' => 'error'
            ); 
            return redirect()->back()->with($notification);   
